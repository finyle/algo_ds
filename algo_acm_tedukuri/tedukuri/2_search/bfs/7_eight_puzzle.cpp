...
int astar(int sx, int sy, int e) {
    ...
    while (q.size()) {
        int now = q.top().state; q.pop(); // 取出堆顶
        if (now == e) return d[now]; // 第一次取出目标状态时，得到答案
        int a[3][3];
        pair<int, int> space = recover(now, a); //复原九宫格
        int x = space.first, y = space.second;
        for (int i = 0; i < 4; i++) { // 枚举空格的移动方向(上下左右)
            int nx = x + dx[i], ny = y + dy[i];
            if (nx < 0|| nx > 2 || ny < 0 || ny > 2) continue;
            swap (a[x][y], a[nx][ny]);
            int next = calc(a);
            if (d.find(next) == d.end() || d[next] > d[now] + 1) { // next状态没有访问过，或者能被更新
                d[next] = d[now] + 1;
                f[next] = now; // f 和 go记录移动的路线，一边输出方案
                go[next] = i;
                q.push(rec(next, d[next] + value(a))); // 入堆
            }
            swap(a[x][y], a[nx][ny]);
        }
    }
